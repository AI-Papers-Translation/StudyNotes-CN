...
* λ 是正则化系数
* 平方和误差函数
  * Eq_3.26 : $E_D(\text{w})=\frac12\sum_{n=1}^N\{t_n-\text{w}^T\phi(\text{x}_n)\}^2$
  * Eq_3.27 : $E_D(\mathbf{w})+\lambda E_W(\mathbf{w})=\frac12\sum_{n=1}^N\{t_n-\text{w}^T\phi(\text{x}_n)\}^2 + \frac{\lambda}2\text{w}^T\text{w}$
...
* 频率学家的角度是 "偏置—方差" 折中 (bias-variance trade-off)
...
* 公式推导
  * 单个数据集，单个数据点 $\text{x}$
    * $y(\text{x}) - h(\text{x}) = \{y(\text{x};\mathcal{D}_c) - \mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)] + \mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)] - h(\text{x})\}^2$
    * $y(\text{x}) - h(\text{x}) = \{y(\text{x};\mathcal{D}_c) - \mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)]\}^2 + \{\mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)] - h(\text{x})\}^2 \\ + 2\{y(\text{x};\mathcal{D}_c) - \mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)]\}\{\mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}_c)] - h(\text{x})\}$
  * 多个数据集，单个数据点 $\text{x}$，求期望
    * $\mathbb{E}_{\mathcal{D}}[\{ y(\text{x};\mathcal{D}_c) - h(\text{x}) \}^2] = \{\mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}) - h(\text{x})]\}^2 + \mathbb{E}_{\mathcal{D}}[\{ y(\text{x};\mathcal{D}) -\mathbb{E}_{\mathcal{D}}[y(\text{x};\mathcal{D}]) \}^2] + 0$
...
* 前提条件
  * 似然函数： ( 噪声精度参数β为已知常数 ) $p(\mathbf{t}|\text{X,w},\beta)=\prod_{n=1}^N \mathcal{N}(\mathbf{t}_n|\text{x}^T\phi(\text{x}_n),\beta^{-1})\tag{3.10}$
  * 对应的共轭分布是高斯分布： $p(\text{w})=\mathcal{N}(\text{w}|\text{m}_0,\text{S}_0)$
    * $\text{m}_0$为均值
    * $\text{S}_0$为协方差
* 公式推导
  * 后验概率
    * Eq_3.49 : $p(\text{w}|\mathbf{t}) = \mathcal{N}(\text{w}|\text{m}_N,\text{S}_N)$
    * Eq_3.50 : $\text{m}_N = \text{S}_N(\text{S}_0^{-1}\text{m}_0 + \beta\Phi^T\mathbf{t})$
    * Eq_3.51 : $\text{S}_N^{-1} = \text{S}_0^{-1} + \beta\Phi^T\Phi$

例子：特定形式的先验 $p(\text{w}|\alpha)=\mathcal{N}(\text{w}|\boldsymbol{0},\alpha^{-1}\mathbf{I})$
...
* 后验概率的对数
  * $\ln p(\text{w}|\mathbf{t})=-\frac\beta2\sum_{n=1}^N\{t_n-\text{w}^T\phi(\text{x}_n)\}^2 - \frac\alpha2\text{w}^T\text{w} + const$
  * $\ln p(\text{w}|\mathbf{t})=-\frac12\sum_{n=1}^N\{t_n-\text{w}^T\phi(\text{x}_n)\}^2 - \frac{\alpha}{\beta}\text{w}^T\text{w} + const$
  * Eq_3.27 : $E_D(\mathbf{w})+\lambda E_W(\mathbf{w})=\frac12\sum_{n=1}^N\{t_n-\text{w}^T\phi(\text{x}_n)\}^2 + \frac{\lambda}2\text{w}^T\text{w},\lambda = \frac\alpha\beta$
  * w 的极值：最大化 ( 后验分布 ) 等价于 最小化 ( 平方和误差函数 + 二次正则化项 )
  * 图 3.7 给出了贝叶斯学习的效果，以及基于贝叶斯的顺序学习的本质

高斯先验分布的推广 (q=2 表示高斯分布)
$$
p(\text{w}|\alpha)=[\frac{q}2(\frac\alpha2)^{\frac1q}\frac1{\Gamma(\frac1q)}]^M\exp(-\frac\alpha2\sum_{j=0}^{M-1}|w_j|^q)\tag{3.56}
$$
...
* 公式推导
  * $p(t|\mathbf{t},\alpha,\beta) = \int p(t|\text{w},\beta)p(\text{w}|\mathbf{t},\alpha,\beta)\text{dw}$
    * $\mathbf{t}=(t_1,\cdots,t_N)^T$：训练数据的目标变量的值组成的向量
    * 目标变量的条件概率分布：$p(t|\text{x,w},\beta)=\mathcal{N}(t|y(\text{x,w},\beta^{-1}))\tag{3.8}$
  * $p(t|\text{x},\mathbf{t},\alpha,\beta) = \mathcal{N}(t|\text{m}_N^T\phi(\text{x}),\sigma_N^2(\text{x}))$
    * Eq_3.59 : $\sigma_N^2(\text{x})=\frac1\beta+\phi(\text{x})^T\text{S}_N\phi(\text{x})$
    * $\frac1\beta$ ：表示数据中的噪声
    * $\phi(\text{x})^T\text{S}_N\phi(\text{x})$ ：表示了与参数 $\text{w}$ 关联的不确定性，当 $N\rightarrow\infty$ 时，超于 0
* 图 3.8：每个点的预测方差与 $x$ 的函数关系
  * 预测的不确定性依赖于数据 $x$，并且在数据点的领域内最小。
  * 不确定性的程度随着观测数据的增多而逐渐减小
* 图 3.9：不同数据点 $x$ 的预测之间的协方差

### Sec_3.3.4. 等价核 <!--TODO:如何理解核方法？-->
...
* $y(\text{x},\text{m}_N)=\text{m}_N^T\phi(\text{x})=\beta\phi(\text{x})^T\text{S}_N\Phi^T\mathbf{t} = \sum_{n=1}^N\beta\phi(\text{x})^T \text{S}_N\phi(\text{x}_n)\mathbf{t}=\sum_{n=1}^N k(\text{x},\text{x}_n)t_n$
  * Eq_3.3  : $y(\text{x,w})=\sum_{j=0}^{M-1} w_j\phi_j(\text{x})=\text{w}^T\phi(\text{x})$
  * Eq_3.49 : $p(\text{w}|\mathbf{t})=\mathcal{N}(\text{w}|\text{m}_N,\text{S}_N)$
  * Eq_3.53 : $\text{m}_N = \beta\text{S}_N\Phi^T\mathbf{t}$
  * Eq_3.51 : $\text{S}_N^{-1}=\text{S}_0^{-1}+\beta\Phi^T\Phi$
  * 等价核：$k(\text{x},\text{x}')=\beta\phi(\text{x})^T \text{S}_N\phi(\text{x}')$
...
## Sec_3.4. 基于贝叶斯方法的模型比较 <!--TODO:寻找比较的案例-->
...
* 前提条件
  * $L$ 个模型 $\{\mathcal{M}_i\}$ 的比较
  * 模型：观察数据集 $\mathcal{D}$ 上的概率分布
  * 后验分布： $p(\mathcal{M}_i|\mathcal{D})\propto p(\mathcal{M}_i)p(\mathcal{D}|\mathcal{M}_i)$
* 模型证据 (model evidence) ：表达了数据展现出的不同模型的优先级。
  * $p(\mathcal{D}|\mathcal{M}_i)=\int p(\mathcal{D}|\text{w},\mathcal{M}_i)p(\text{w}|\mathcal{M}_i)\text{dw}$
  * 也叫边缘似然 (marginal likelihood)。
  * 贝叶斯因子 (Bayes factor) ：是两个模型的模型证据的比值。
    * $\frac{p(\mathcal{D}|\mathcal{M}_i)}{p(\mathcal{D}|\mathcal{M}_j)}$
  * 模型有一个参数的情形
    * $p(\mathcal{D})=\int p(\mathcal{D}|\text{w})p(\text{w})\text{dw}\simeq p(\mathcal{D}|\text{w}_{{}_{MAP}})\frac{\Delta\text{w}_{后验}}{\Delta\text{w}_{先验}}$
    * $\ln p(\mathcal{D})\simeq\ln p(\mathcal{D}|\text{w}_{{}_{MAP}}) + \ln (\frac{\Delta\text{w}_{后验}}{\Delta\text{w}_{先验}})$
    * $\ln p(\mathcal{D}|\text{w}_{{}_{MAP}})$ : 基于最可能的参数（MAP）拟合数据
    * $\ln (\frac{\Delta\text{w}_{后验}}{\Delta\text{w}_{先验}})$ : 根据模型的复杂度来惩罚模型，即 $\lambda=(\frac{\Delta\text{w}_{后验}}{\Delta\text{w}_{先验}})$
  * 模型有 M 个参数的情形
    * $\ln p(\mathcal{D})\simeq\ln p(\mathcal{D}|\text{w}_{{}_{MAP}}) + M \ln (\frac{\Delta\text{w}_{后验}}{\Delta\text{w}_{先验}})$
    * 复杂度惩罚项的大小随着模型中可调节参数 M 的数量线性增加
    * 模型复杂度增加时，$\ln p(\mathcal{D}|\text{w}_{{}_{MAP}})$ 会变大，因为模型越复杂，拟合度越好。
...
* 模型选择 ( model selection ) ：对于模型求平均的简单近似是使用最有可能的模型做预测。
  * 简单的模型拟合数据的效果较差
  * 复杂的模型的预测概率分散到过多的可能的数据集当中，从而每个数据集赋予的概率都相对较小。
* 贝叶斯模型比较
...
* 公式推导
  * Eq_3.74 : $p(t|\mathbf{t})=\int\int\int p(t|\text{w},\beta)p(\text{w}|\mathbf{t},\alpha,\beta)p(\alpha,\beta|\mathbf{t})\text{dw dα dβ}$
  * Eq_3.8  : $p(t|\text{x,w},\beta)=\mathcal{N}(t|y(\text{x,w}),\beta^{-1})$
  * Eq_3.49 : $p(\text{w}|\mathbf{t},\alpha,\beta) = \mathcal{N}(\text{w}|\text{m}_N,\text{S}_N)$
  * Eq_3.53 : $\text{m}_N = \beta\text{S}_N\Phi^T\mathbf{t}$
  * Eq_3.54 : $\text{S}_N^{-1}=\alpha\mathbf{I} + \beta\Phi^T\Phi$
  * 如果 $\alpha=\hat{\alpha},\beta=\hat{\beta}$，则 $p(t|\mathbf{t})\simeq p(t|\mathbf{t},\hat{\alpha},\hat{\beta}) = \int p(t|\text{w},\hat{\beta})p(\text{w}|\mathbf{t},\hat{\alpha},\hat{\beta})\text{dw}$
...
* 缺点：因为基函数在观测到任何数据之前就被固定下来，因此基函数的数量会随着输入空间的维度 $D$ 的增长而呈指数方式增长。 ( 参考 Sec_1.4.)
