...
$$
\mathbb{E}[\text{x}] = \frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \int\exp\{-\frac12 ( \text{x} - \boldsymbol{\mu} ) ^T \Sigma^{-1} ( \text{x}-\boldsymbol{\mu} ) \} \text{x dx}
$$

$$
\mathbb{E}[\text{x}\text{x}^T] = \frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \int\exp\{-\frac12 ( \text{x} - \boldsymbol{\mu} ) ^T \Sigma^{-1} ( \text{x}-\boldsymbol{\mu} ) \} \text{xx}^T\text{ dx}
$$
...
$$
\mathbb{E}[\text{x}] = \frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \int\exp\{-\frac12 \text{z}^T \Sigma^{-1} \text{z}\} ( \text{z}+\boldsymbol{\mu} ) \text{dz}
$$

$$
\mathbb{E}[\text{x}\text{x}^T] = \frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \int\exp\{-\frac12 \text{z}^T \Sigma^{-1} \text{z}\} ( \text{z}+\boldsymbol{\mu} ) ( \text{z}+\boldsymbol{\mu} ) ^T\text{dz}
$$

$$
\frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \int\exp\{-\frac12 \text{z}^T \Sigma^{-1} \text{z}\}\text{zz}^T \text{dz} \\ = \frac1{ ( 2\pi ) ^{D/2}} \frac1{|\Sigma|^{1/2}} \sum_{i=1}^D\sum_{j=1}^D \text{u}_i\text{u}_j^T \int\exp\{-\sum_{k=1}^D\frac{y_k^2}{2\lambda_k}\}y_i y_j \text{dy} =\sum_{i=1}^D \text{u}_i\text{u}_j^T\lambda_i = \Sigma
$$
...
-   基于「完成平方项」的「配平方法」: 与常规的高斯分布 $\mathcal{N} ( \text{x}|\boldsymbol{\mu},\Sigma ) $ 的指数项对应
...
$$
\int\exp\{-\frac12 ( \text{x}_b - \Lambda_{bb}^{-1}\text{m} ) ^T\Lambda_{bb} ( \text{x}_b - \Lambda_{bb}^{-1}\text{m} ) \}\text{ dx}_b
$$
基于标准的多元高斯概率分布公式 ( Eq 2.43 )，再次使用配平方法

将 $\text{x}_b$ 积分得： <!--TODO-->

 ( Eq 2.84 ) 和 ( Eq 2.70 ) 中与 $\text{x}_a$ 相关的项相加，$const$ 表示与  $\text{x}_a$ 无关的项
$$
\frac12\text{m}^T\Lambda_{bb}^{-1}\text{m}
-   \frac12\text{x}_a\Lambda_{aa}\text{x}_a
+ \text{x}_a^T ( \Lambda_{aa}\boldsymbol{\mu}_a + \Lambda_{ab}\boldsymbol{\mu}_b ) + const \\
= \frac12
...
继续使用配平方法，得出边缘概率分布 $p ( \text{x}_a ) $ 的参数
...
-   配平方法，寻找 ( Eq 2.102 ) 中的线性项与 ( Eq 2.71 ) 配对，再基于$\text{cov [ z]}$得
...
-   对数似然函数
    -   $\ln p ( \text{X}|\boldsymbol{\mu},\Sigma ) =-\frac{ND}{2}\ln ( 2\pi ) -\fracN2\ln|\Sigma|-\frac12\sum_{n=1}^N ( \text{x}_n-\boldsymbol{\mu} ) ^T\Sigma^{-1} ( \text{x}-\boldsymbol{\mu} ) $
-   高斯分布的充分统计量
    -   $\sum_{n=1}^N\text{x}_n$
...
## 2.5. 非参数化密度估计
 ...
 ( 这个不是本书重点，作者描述的不多，深入了解可参考 [^Andrew,2004 ] Ch 03 和 [^Duda,2003 ] Ch 04 )
...
-   概率密度建模的非参数化方法
    ...
    -   密度估计的核密度方法
        -   固定 V，确定 K
        -   为了得到光滑的模型，选择一个核函数 ( kernel function )，即 Parzen 窗。
        -   不需要进行训练阶段的计算
